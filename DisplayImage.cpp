...
double* DROPOUT2 = (double*)malloc(sizeof(double) * n);
...
int Dense_relu(const int hidden_layer_A,const int hidden_layer_B,double hidden_layer_A_arr[],double hidden_B_arr[],double AK[],int temp){
	...
	for (i = 0; i < hidden_layer_B; i++) {
		...
		x_sum += AK[temp +  j];
		hidden_B_arr[i] = leaky_relu(x_sum);
...
int input_layer_relu(const int hidden_layer_A,double hidden_layer_A_arr[],double AK[],int temp){
	...
	for (i = 0; i < hidden_layer_A; i++) {
		hidden_layer_A_arr[i] = leaky_relu(AK[i * 2 + 0] * x_input[i] + AK[i * 2 + 1]);
...
double CE(double x,int y){  //cross entropy
	if(y==1){
		return -log(x);
	}
	else{
...
double FL(double x,int y){  //Focal Loss
	double r=2;
	double a=0.75;
	if(y==1){
		double p=x;
		return -a*pow((1-p),r)*log(p);
	}
	else{
		double p=1-x;
		return -a*pow((1-p),r)*log(p);
...
double partialW(double A[], double  x_input[], double y, int N) {
	...
	return (loss(F(A1, x_input),y)-loss( F(A2, x_input),y))/(2*h);
...
int main() {
	...
	*/
	...
	auto start = std::chrono::system_clock::now();
		...
		for (rrr = 0; rrr < 16; rrr++) {
			...
			int kkk = 0;
			char str[] = "./5/";
			String str2 = std::to_string(rand() % int(data*0.5));
			char str3[] = ".png";
			String Str;
			Str.append(str);
			Str.append(str2);
			...
			char strB[] = "./5/";
			String str2B = std::to_string(rand() % int(data*0.5));
			char str3B[] = ".png";
			String StrB;
			StrB.append(strB);
			StrB.append(str2B);
		...
		for(i=0;i<20;i++){
			char str[] = "./5/";
			String str2 = std::to_string(data + rand() % int(data2*0.5));
			char str3[] = ".png";
			String Str;
			Str.append(str);
			Str.append(str2);
			...
			if(F(A, x_input)<0.5)acc_test+=1;
			
			
			char strA[] = "./8/";
			String str2A = std::to_string(data + rand() % data2);
			char str3A[] = ".png";
			String StrA;
			StrA.append(strA);
			StrA.append(str2A);
	...
	*/
	...
	while (true)
	{
		...
		int input;
		printf("input image(5 is 5，8 is 8):");
		scanf("%d", &input);
		if (input == 5) {

			char strC[] = "./5/";
			String str2C = std::to_string(data + rand() % int(data2*0.5));
			char str3C[] = ".png";
			String StrC;
			StrC.append(strC);
			StrC.append(str2C);
		...
		if (input == 8) {
			...
			char strC[] = "./8/";
			String str2C = std::to_string(data + rand() % data2);
			char str3C[] = ".png";
			String StrC;
			StrC.append(strC);
			StrC.append(str2C);
